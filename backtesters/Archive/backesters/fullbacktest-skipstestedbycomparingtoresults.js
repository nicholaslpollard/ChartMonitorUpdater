...
// ---------------- Alpaca API Setup ----------------
// Initialize Alpaca client with API keys, paper trading mode, and base URL
...
// ---------------- Results File Setup ----------------
// Load previous results.json if exists, otherwise create new; stores all backtest results
let allResults = [];
...
// ---------------- Delay Helper ----------------
// Sleep function used for rate-limiting and spacing API calls
const sleep = (ms) => new Promise((res) => setTimeout(res, ms));

// ---------------- Fixed Delay / Concurrency ----------------
// Set API delay, max concurrent stock processing, and queues for retries
...
// Handles API rate-limits (429) by pausing all activity for 25 seconds
...
// Fetch OHLCV data from Alpaca, with retries and 429 handling
...
// Compute stop-loss and target prices based on ATR and trade direction
...
// Simulate trades over historical data, compute trades, wins/losses, avg duration, and reward-to-risk
...
// Store results in memory and write continuously to results.json
function saveResult(symbol, result) {
  ...
  //console.log(`Saved results for ${symbol}`);  For if confirmation of logging if errors
}

// ---------------- Update Optionable Stock List ----------------
// Checks CSV last modified; updates asynchronously if older than 24h
...
// Evaluate all strategies for one stock, keep the one with highest average win rate
async function processStockConcurrent(stock, strategies, start, end) {
  // ✅ Skip stock if already exists in results.json
  if (allResults.some(r => r.symbol === stock.symbol)) {
...
// Process multiple stocks concurrently; rerun 0% win-rate stocks on 1-year data
...
// ---------------- Main ----------------
// Load strategies, update optionable stock CSV, filter tradable stocks, add SPY if missing, and run concurrent backtests
(async () => {
  try {
